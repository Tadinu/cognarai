...
class AllegroScrewdriver(AllegroValveTurning):
    def get_constraint_dim(self, T):
        ...
        self.dz = (
                      self.friction_polytope_k) * self.num_fingers + 1 + self.num_fingers  # one friction constraints per finger, num_finger means minimum force constraint
    ...
    def __init__(self,
                 ...
                 chain,
                 object_location,
                 # the position of the root joint used to compute the torque balance, it's not necessarily the asset root pose.
                 object_type,
                 world_trans,
                 object_asset_pos,
                 object_urdf_path,
    ...
    def get_initial_xu(self, N):
        ...
        force[:, :, :3] = force[
                              :, :, :3] * 0.01  # NOTE: scale down the index finger force, might not apply to situations other than screwdriver
    ...
    def _index_contact_region_constraint(self, xu, compute_grads=True, compute_hess=False):
        ...
        h = self.index_contact_region_constr(index_contact_pts.reshape(-1, 3), env_q.reshape(-1, self.obj_dof)).reshape(
            N, -1)
        if compute_grads:
            grad_h = torch.zeros(N, 1, T, T, d, device=self.device)
            dh_dcontact, dh_denv_q = self.grad_index_contact_region_constr(index_contact_pts.reshape(-1, 3),
                                                                           env_q.reshape(-1, self.obj_dof))
            ...
            grad_h[:, :, T_range, T_range, self.robot_dof: self.robot_dof + self.obj_dof] = dh_denv_q.reshape(N, T, 1,
                                                                                                              self.obj_dof).transpose(
                1, 2)
    ...
    def _friction_vel_constraint(self, xu, finger_name, compute_grads=True, compute_hess=False):
        ...
        if compute_grads:
            ...
            grad_h[:, :, T_range_plus, T_range_minus, self.robot_dof: self.robot_dof + self.obj_dof] = dh_dtheta[
                :, 1:].transpose(1, 2)
            grad_h[:, :, T_range, T_range, self.dx: self.dx + self.robot_dof] = dh_du.reshape(N, T, dh,
                                                                                              self.robot_dof).transpose(
                1, 2)
            grad_h = grad_h.transpose(1, 2).reshape(N, -1, T * d)
        else:
            return h, None, None
    ...
    def check_validity(self, state):
        ...
        screwdriver_top_pos = get_screwdriver_top_in_world(screwdriver_state, self.object_chain, self.world_trans,
                                                           self.object_asset_pos)
