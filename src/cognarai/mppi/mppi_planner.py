...
from cognarai.mppi.mppi_torch import MPPITorch
...
class OmniMPPIPlanner(object):
    ...
    def dynamics(self, _, u) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    def terminal_cost(self, states, actions):
        terminal_scale = 100
        return terminal_scale * self.running_cost(states[..., -1, :])
    ...
    def compute_action_tensor(self, dof_state_data: bytes, root_state_data: bytes):
        ...
        action = self.mppi.command(self.state_place_holder) if self.mppi else \
            self.mppi_kernel.command(self.state_place_holder)
        if action.dim() == 1:
...
def main(task_base_cfg: MPPITaskBaseCfg):
    ...
    planner = zerorpc.Server(OmniMPPIPlanner(task_cfg))
