...
def accumulate_running_cost(running_cost, terminal_cost):
    def _accumulate_running_cost(x, u):
        # cost[:, -1] += total_cost
        return running_cost(x, u) + terminal_cost(x, u)
...
class ICEM:
    ...
    def __init__(self, dynamics, trajectory_cost, nx, nu, sigma=None, num_samples=100, num_elites=10, horizon=15,
        ...
        self.N = num_samples
        self.device = device
        #self.cost_total: torch.Tensor = torch.zeros(num_samples, device=self.device, dtype=sigma.dtype)
        ...
        self.sigma = self.sigma.detach().clone()
    ...
    def command(self, state, shift_nominal_trajectory=True, return_full_trajectories=False, **kwargs):
        ...
        if return_full_trajectories:
            ...
            out_trajectory = torch.cat((out_X, out_U), dim=-1) if out_X is not None else None
