...
class IsaacWorld(object):
    ...
    def spawn_object(self, path: str, position: np.ndarray = [0,0,0], orientation: np.ndarray=[0,0,0,1]) -> int:
        if path is None:
            raise ValueError("Path to the object file is required.")

        # Load object configs, including kinematics
        entity_model_name = Path(path).stem
        entity_prim = None
        entity_id = -1
        isaac = Isaac()
        isaac.load_entity_config(path)
        ...
        elif path.endswith('usd') or path.endswith('.usda') or path.endswith('.usdc'):
            entity_id = len(self.isaac.objects[entity_model_name]) if entity_model_name in self.isaac.objects else 0
            entity_prim_path = f"/World/{entity_model_name}{entity_id}"
            isaac.spawn_object(world=self.omni_world,
                               object_model_name=entity_model_name,
                               object_prim_path=entity_prim_path,
                               position=position)
        else:
            logging.error(f"[{entity_model_name} - {path}] entity model is not supported")
