...
from cognarai.fabrics.components.robot_representation import RobotRepresentation
from cognarai.fabrics.components.environment import Environment
...
class FabricPlannerConfig:
    ...
    damper_beta: str = (
        "0.5 * (ca.tanh(-0.5 * (ca.norm_2(x) - 0.02)) + 1) * 0.65 + 0.01 + ca.fmax(0, sym('a_ex') - sym('a_le'))"
    )
    damper_eta: str = (
