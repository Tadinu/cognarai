...
from allegro_cuboid_turning import AllegroCuboidTurning
from allegro_cuboid_alignment_w_force import AllegroCuboidAlignment
from allegro_reorientation import AllegroReorientation
from allegro_env import AllegroManipEnvCfg, AllegroManipEnv
...
def do_trial(env: AllegroManipEnv, params, fpath):
    ...
    if pregrasp_flag:
        ...
        while pregrasp_succ == False:
            ...
            pregrasp_problem = AllegroContactProblem(
                ...
                object_asset_pos=env.object_pos,
    ...
    if task == 'screwdriver_turning':
        manipulation_problem = AllegroScrewdriver(
            ...
            chain=params['chain'],
            device=params['device'],
            object_asset_pos=env.object_init_pos,
        ...
        manipulation_problem = AllegroCuboidAlignment(
                        ...
                        chain=params['chain'],
                        device=params['device'],
                        cuboid_asset_pos=env.object_init_pos,
    ...
    for k in range(params['num_steps']):
        ...
        action_full = torch.zeros(1, env.action_space.shape[1], dtype=action.dtype, device=action.device)
        action_full[:, env.actuated_dof_indices] = action
...
def main(num_envs: int = 10):
    # get config
    args.task = 'cuboid_turning'
    if args.task == 'screwdriver_turning':
        config = yaml.safe_load(pathlib.Path(f'{CCAI_PATH}/config/allegro_screwdriver.yaml').read_text())
        config['obj_dof_code'] = [0, 0, 0, 1, 1, 1]        
        config['num_env_force'] = 1
    elif args.task == 'valve_turning':
        config = yaml.safe_load(pathlib.Path(f'{CCAI_PATH}/config/allegro_valve.yaml').read_text())
        config['obj_dof_code'] = [0, 0, 0, 0, 1, 0]
        config['num_env_force'] = 0
    elif args.task == 'cuboid_turning':
        config = yaml.safe_load(pathlib.Path(f'{CCAI_PATH}/config/allegro_cuboid_turning.yaml').read_text())
        config['obj_dof_code'] = [1, 1, 1, 1, 1, 1]
        config['num_env_force'] = 0
    elif args.task == 'cuboid_alignment':
        config = yaml.safe_load(pathlib.Path(f'{CCAI_PATH}/config/allegro_cuboid_alignment.yaml').read_text())
        config['obj_dof_code'] = [1, 1, 1, 1, 1, 1]
        config['num_env_force'] = 1
    elif args.task == 'reorientation':
        config = yaml.safe_load(pathlib.Path(f'{CCAI_PATH}/config/allegro_reorientation.yaml').read_text())
        config['obj_dof_code'] = [1, 1, 1, 1, 1, 1]
        config['num_env_force'] = 0
    ...
    from allegro_env import AllegroCuboidTurningEnv
    #sim, gym, viewer = env.get_sim()
    # IsaacApp
    env = AllegroCuboidTurningEnv(fingers = config['fingers'])
    app = IsaacApp(app_launcher, sim_cfg=env.cfg.sim, args=args)
    app._interactive_scene = env.scene
    results = {}

    # set up the kinematic chain
    chain = pk.build_chain_from_urdf(open(AllegroManipEnvCfg().hand_urdf_path).read())
    ...
    for i in tqdm(range(config['num_trials'])):
        ...
        object_location = env.object_init_pos.to(params['device']).float()  # NOTE: this is true for the tasks we have now. We need to pay attention if the root joint is not the root of the asset
