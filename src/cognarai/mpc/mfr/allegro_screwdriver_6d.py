...
class AllegroScrewdriver6D(AllegroValveTurning):
    def __init__(self,
                 start,
                 goal,
                 T,
...
def do_trial(env, params, fpath, sim_viz_env=None):
    ...
    for k in range(params['num_steps']):
        ...
        planned_theta_traj = best_traj[
            :, 4 * num_fingers_to_plan: 4 * num_fingers_to_plan + obj_dof].detach().cpu().numpy()
        ...
        if params['visualize_plan']:
            ...
            visualize_trajectory(traj_for_viz, turn_problem.contact_scenes['thumb'], viz_fpath, turn_problem.fingers,
        ...
        if params['optimize_force']:
            print("planned force")
            print(action[:, 4 * num_fingers_to_plan:].reshape(num_fingers_to_plan,
                                                              3))  # print out the action for debugging
        # print(action)
...
if __name__ == "__main__":
    ...
    env = AllegroScrewdriverEnv(num_envs=1, control_mode='joint_impedance',
    ...
    asset = f'{ALLEGRO_URDF_DIR}/allegro_hand_right.urdf'
    ee_names = {
    ...
    for i in tqdm(range(config['num_trials'])):
        ...
        for controller in config['controllers'].keys():
            ...
            object_location = torch.tensor(env.table_pose).to(
                device).float()  # TODO: confirm if this is the correct location
