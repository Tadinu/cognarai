...
class MPPITorch(ABC):
    ...
    def _dynamics(self, state: torch.Tensor, u: torch.Tensor, t=None):
    ...
    def command(self, state) -> torch.Tensor:
        """
        ...
        if self.mppi_mode == 'simple':
            ...
            # Update [self.cost_total, .mean_action] here-in
            self._compute_total_cost_batch_halton()
